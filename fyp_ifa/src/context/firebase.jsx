...
export const FirebaseProvider = (props) => {
  ...
  const addReportToCollection = async (PatientId, testName, testFile) => {
    const fileRef = ref_storage(
      storage,
      `uploads/TestFiles/${Date.now()}-${testFile}`
    );
  ...
  const uploadFilesToFirestore = async (data, docs) => {
    ...
    for (let i = 0; i <= docs.length; i = i + 1) {
      ...
      const imageRef = ref_storage(
        storage,
        `uploads/document/${Date.now()}-${fileName}`
      );
  ...
  const update = async (patientID) => {
    const docRef = doc(db, "Patients", patientID);
    const newDeviceToken = window.prompt(
      "Please enter the DeviceToken you want to update"
    );
    if (newDeviceToken) {
      try {
